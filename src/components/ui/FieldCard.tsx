...
export function FieldCard({ field, variants }: FieldCardProps) {
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-primary/95 via-primary/30 to-transparent transition-all duration-500 ease-out group-hover:from-primary group-hover:via-primary/70" />

      {/* Optional Shimmer effect overlay element */}
      <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-transparent opacity-0 transition-opacity duration-700 group-hover:opacity-100 mix-blend-overlay" />
      ...
      <div className="absolute inset-0 p-6 flex flex-col justify-end items-center text-center">
        ...
        {/* Subtle accent dot or line that appears under text */}
        <div className="w-0 h-[2px] bg-gold mt-3 rounded-full transition-all duration-500 ease-out group-hover:w-8" />
      </div>
    </motion.div>
  );
}
